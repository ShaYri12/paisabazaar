...
export default function Footer() {
  ...
  return (
    <>
      <div className='p-[18px] bg-[#60bb51]'>
              ...
              <div key={index}>
                ...
                <ul className="space-y-0">
                  {category.links.map((link, linkIndex) => (
                    <li key={linkIndex} className='py-2 text-[#979797] hover:text-white text-xs'>
                      <Link href="#" className="hover:border-b-[1.2px] hover:border-[white] pb-1 transition duration-300">
                        {link}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="mt-6 pt-5 md:px-4 pb-[22px] border-y border-dotted border-[#777]">
            <div className="flex items-center flex-wrap  gap-4 text-xs">
              ...
              {otherLinks.map((link, index) => (
                <Link key={index} href="#" className="hover:text-white pb-0.5 text-[#979797] hover:border-b-[1.2px] hover:border-white transition duration-300">
                  {link}
                </Link>
              ))}
            </div>
          ...
          <div className='flex lg:flex-row flex-col items-start lg:items-center justify-between'>
            ...
            <div className="mt-10 flex items-start lg:items-center lg:flex-row flex-col justify-between gap-5 md:gap-8 md:px-[15px]">
              <div className=''>
                ...
                <p className='py-2.5 text-xs text-[#979797] flex items-center gap-2'><MdEmail className='bg-white p-[3px] text-xl rounded-full text-black' />support[at]merimudra[dot]com</p>
              </div>
            ...
            <div className="mt-8 flex justify-center items-center flex-wrap md:gap-0 gap-3 w-full lg:w-fit">
              {bottomLinks.map((link, index) => (
                <Link key={index} href="#" className="px-[15px] text-xs text-[#979797]">
                  {link}
                </Link>
              ))}
            </div>
